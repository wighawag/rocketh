...
- **An extension package's ROOT export surface may contain ONLY curried `(env) => …` functions** (plus `export type`s, which erase). The documented user idiom is a namespace spread (`const extensions = {...deployExtension, ...myExtension}`), and `withEnvironment` calls `value(env)` on EVERY entry it is given: a re-exported CLASS or a plain constant is refused BY NAME (`extension entry "Foo" is a class, which cannot be called with the environment`). The failure still surfaces at deploy-script RUN time, not at build time, so the naming is what makes it diagnosable. Put anything else (an error class, a constant) on a subpath export — `@rocketh/unknown-signer` keeps `UnknownSignerError` on `./errors` for exactly this reason.
