...
**A CLI flag turns it on, and names where it goes.** Absent, the run behaves exactly as it does today and writes no new file at all. The flag takes the output PATH: the consumer who needs it is an operator already on the `--is-fork` path, so a flag reaches them without a config change. **Name it for the WRITE, not for the capture**, e.g. `--write-transactions <file>`: capture itself is unconditional and flagless (the previous task), so a name like `--capture-transactions` would assert a capability the flag does not control and would send a plugin author looking for a flag to get the in-process list. That is the muddle `CONTEXT.md` pins against when it explains why the fork flag is `--is-fork` and never `--fork`. Do NOT justify the path form as a way to point at a stream: the write is atomic via write-then-rename, which replaces a FIFO rather than writing into it, so that rationale must not reach the `--help` text.
...
**The file is a contract with consumers outside this process and outside JavaScript** (a Solidity test's `setUp()`, a Safe batching tool), so the serialization has to be parseable by them. How `value` is carried in memory is settled by the previous task: the 0x-quantity wire form the choke point actually receives, never a bigint. Carry it through unchanged rather than inventing a second form, and pin it by a test. The entry union has two arms (`type: 'intent'` and `type: 'raw'`) and the raw arm carries no `signability`; serialise both faithfully rather than flattening them. Absent fields stay absent rather than becoming `null`. Nothing that the entry does not carry may appear in the file: no gas, no fees, no nonce, no hash.
...
- [ ] A SUCCESSFUL run that broadcast nothing writes an EMPTY list rather than leaving a previous file in place, so an operator can never hand a Safe yesterday's batch believing it is today's. rocketh is idempotent, so a repeat rehearsal capturing nothing is the common case, not the corner
...
>
> RECORD non-obvious in-scope decisions in a `## Decisions` block at the end of your FINAL REPORT: the flag's spelling and why, what a successful run that broadcast NOTHING writes and why, how you made the write atomic, and where you put the writer relative to the executor. Do not write the done record, the commit message or the PR body, and do not edit this task file.
>
...
1. **The flag is `--write-transactions <file>`.** It names the WRITE, which is the only thing it controls: capture is unconditional and flagless (`env.capturedTransactions` exists on every run), so `--capture-transactions` would assert a capability the flag does not own and would send a plugin author hunting for a flag to get the in-process list. Alternatives considered: `--capture-transactions` (rejected, above), and `--write-captured-transactions` (rejected as longer without disambiguating: at the CLI there is no other set of transactions a run could write, and _captured_ is the term the in-process/glossary layer uses). This is the same discipline `CONTEXT.md` and ADR 0014 pin for `--is-fork` versus `--fork`. It touches `document-captured-transactions`, which will document the flag and its `--help` wording as shipped.

2. **The option is a `@rocketh/node` parameter, NOT a core one.** The task allowed either and called the Node-local route preferable, and it is available: `resolveExecutionParams` maps core's run parameters field by field with no spread, so an output path would have had to be added to BOTH `ExecutionParams` and `ResolvedExecutionParams` to survive the trip, widening a type every package depends on for a field only the Node runtime could ever read (and that a browser-capable core must never act on, ADR 0002). Instead `@rocketh/node` declares `NodeExecutionParams = ExecutionParams & {writeTransactions?: string}` and the two EXECUTE entry points take it; the environment-only loaders keep core's type, since a run that executes nothing has nothing to write. Core and `rocketh` are unchanged, so the ask-first change `AGENTS.md` warns about did not have to happen. It touches hardhat-deploy only as a widening (an `ExecutionParams` still passes).

3. **The writer sits INSIDE the Node run assembly, after the executor returns.** Specifically in `loadAndExecuteDeploymentsFromFilesWithSpecificConfig`, the single function both public execute entry points funnel through, which is where the environment (and therefore `capturedTransactions`) first exists on the Node side. Not the bin script: `cli.ts` ends with a bare, un-awaited `loadAndExecuteDeploymentsFromFiles(executionParams)` and never holds the returned environment. Not `_executeDeployScriptsFromFiles`, which only ever sees the RESOLVED core parameters. The placement is what makes _only on success_ free: a throw from the scripts propagates past that line, so nothing is written and any previous file is untouched.

4. **A successful run that broadcast NOTHING writes an EMPTY list, `[]`, replacing whatever was at the path.** rocketh is idempotent, so a repeat rehearsal that captures nothing is the common case rather than a corner, and leaving the previous file in place would let an operator hand a Safe yesterday's batch believing it is today's. The alternative (write nothing when the list is empty) makes _no file_ ambiguous between "the run wrote nothing" and "the run failed", which is exactly the distinction the lifecycle exists to keep sharp.

5. **Atomicity is write-then-rename within the target's own directory**, into a dotted temp name (`.batch.json.<uuid>.tmp`) so a consumer watching the directory cannot pick it up as a batch, and the temp file is removed if the write throws. Same directory because `rename` is only atomic within one filesystem. The consequence, stated in the JSDoc and deliberately kept OUT of the `--help` text: the write REPLACES the path, so pointing it at a FIFO is not a supported way to stream a run. A test observes the atomicity rather than describing it (the inode changes on rewrite, which an in-place `writeFileSync` would not do).

6. **The file's top level is a bare JSON array**, two-space indented, with a trailing newline. No envelope, no version field, no run metadata: the file has one job and the whole of it is the ordered list, which a Safe tool or a Solidity `setUp()` can walk directly. An envelope would be a new concept nobody asked for and would make the empty case (`[]`) less obviously "this run sent nothing". This is a user-visible format, so it is a hard-to-reverse default worth naming here.

7. **Each entry is serialized key by key, not passed through.** A field that ever lands on a captured entry (a gas price, a nonce, a hash) therefore cannot reach the file by accident and become a contract with consumers who would replay it. The trade-off accepted: a future legitimate field must be added here deliberately. The `switch` is exhaustive over the two arms with a `never` default, so a third arm fails to compile rather than being silently dropped, mirroring the choke point's signer switch.

8. **An empty `--write-transactions` value is REFUSED, not read as "no file".** This is a new refusal, and it is the opposite of how `--tags ''` is read, deliberately: an empty tag filter has a true meaning (select everything), while an empty path names nothing a file could be written to, and silently skipping the write ends a successful rehearsal with no batch and nothing on screen to say why, which an operator cannot detect. It throws from `resolveWriteTransactionsPath` exactly as `resolveUnknownSignerPolicy` does, so `cli.ts` turns it into a message on stderr and exit 1, BEFORE the run rather than after a rehearsal has completed. It touches the CLI's option surface only.

9. **The writer creates the parent directory when missing**, so `--write-transactions out/batch.json` is the user's answer rather than an ENOENT discovered after the whole rehearsal has run. It mirrors what the FS deployment store already does for a deployments folder.

10. **I added one row to `packages/rocketh-node/README.md`'s options table.** That table is this package's reference for the option surface, so shipping a flag without it is drift on day one. I deliberately added no prose section: the user-facing page, the glossary entry and the `--help` documentation belong to `document-captured-transactions`, which is the task that owns them.
