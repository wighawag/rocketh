...
- **Where the pairs live: `packages/hardhat-deploy/test/migration-pairs/`.** Why: `hardhat-deploy` is the package a v1 user migrates to, and the skill is named after it. It already had `rocketh` as a dev dependency, so the ports use the real `setupDeployScripts` and read exactly like user scripts (`import {deployScript, artifacts} from '../rocketh/deploy.js'`). Its tests already run under `pnpm test`. Alternatives considered:
  - `demoes/`: not run by `pnpm test`, and its needs a real node.
  - `@rocketh/proxy` or `@rocketh/unknown-signer` (where the precedent lives): neither has the diamond dependency, and the pairs are not about either package.
  - A new private package: more structure than the job needs.
  - What it touches: `hardhat-deploy` gains dev dependencies on `@rocketh/deploy`, `@rocketh/diamond`, `@rocketh/proxy`, `@rocketh/read-execute`, `@rocketh/test-utils` (all `workspace:*`) and `viem@^2.55.13`. No new third-party package enters the tree. AGENTS.md makes new dependencies ask-first, so please ratify.
- **v1 halves are `*.v1.ts` files excluded in `tsconfig.json`.** Why: they stay formatted and readable as TypeScript but are never compiled. Alternative: `// @ts-nocheck` in every v1 file, rejected as noise in the text the skill copies. What it touches: only `packages/hardhat-deploy/tsconfig.json`.
- **The skill includes the files verbatim, guarded by `skill.test.ts`.** Why: the skill is fetched as a single file, so links alone would leave agents without the code. The four proxy pairs other than the transparent one, the `upgradeIndex` step 0 and the diamond cut are listed by path in the skill instead of copied in full. What it touches: anyone editing the skill must copy from the pair files.
- **Run-once, tags and the fixture go through the real executor.** They run via a stand-in `rocketh/environment.ts` that calls `createExecutor` over the scripts imported directly, with the provider coming from `createTestEnvironment`. Every other pair runs as `script(env)` on a `createTestEnvironment` environment, which is what the executor itself does. Why: those three behaviours belong to the executor, and copying its loop into the test would test the copy. The stand-in replaces only `@rocketh/node`'s file and config loading, and says so at the top of the file.
- **"Recompiling" is modelled by swapping one entry of a mutable `artifacts` object, restored in `afterEach`.** This is how pair 5 re-runs an unchanged script after its contract changed, which is the natural v1 way to trigger `onUpgrade`.
- **The test's mock chain applies upgrade calls to the implementation slot, and records what each proxy's constructor wrote.** Proxy constructor state comes from `chain.constructProxies(env)`, which reads it back from each proxy record's constructor arguments. Why: without this, a re-run has no on-chain state to compare against. It extends the precedent's `createChain` approach.
- **The diamond pair keeps `execute` in the edited script.** That matches v1, where the init call runs on every cut, and the skill states it plainly rather than hiding it.
