...
- **The filesystem store is a copy inside the test file, not the one from `@rocketh/node`.** `createFSDeploymentStore` isn't exported from `@rocketh/node`'s public entry, and its package exports block deep imports. Importing its source by relative path fails typecheck with TS6059 (file outside `rootDir`), so it would have meant widening this package's `rootDir` and pulling another package's sources into this one's type-check. The copy keeps the same on-disk layout, including the `.chain` marker. The alternatives were exporting the store from `@rocketh/node` (a public API change plus a changeset, out of scope) or widening `rootDir`. What this touches: if the real store's layout ever changes, this copy won't follow it. That doesn't weaken the assertion, which is "nothing reaches the disk". The copy's writes are deliberately synchronous, because the environment doesn't wait for its own record writes to finish, and an async write could land after a snapshot was taken.
- **How the snapshot works.** It walks the temp folder recursively and records, sorted, each entry's relative path, whether it is a folder or a file, and each file's full contents. The byte-identical test first puts real files in the folder and asserts they are there, so an empty "after" snapshot can't pass by accident.
- **The test loads records through `internal.loadDeployments()`.** `createTestEnvironment` doesn't load records from the store; the executor does that at the start of a run. The test calls the same internal method so the deferring run starts from a populated state, as a real repo would.
- **Home and repo checks are existence checks.** Snapshotting the whole home folder isn't feasible, since other processes change it all the time. So the test checks that no `deployments/` folder appears in the places a wrongly resolved default would land. It combines that with a full snapshot of the package tree (skipping `node_modules`, `dist` and `.turbo`) and a check that every store call names a folder under the temp dir.
- **Helpers are copied, not shared.** `safeOwnerEnvironment` and `upgradeCall` are copied from `catchUnknownSigner.integration.test.ts` so the parity contract reads on its own in one file. Some assertions from that existing file are repeated on purpose for the same reason, and the file's header says so.
- **The printed block is pinned as exact text.** The task's note at the top says to pin the v1 block, not all of the output. A wording change to that block will therefore need a deliberate update to this test.
