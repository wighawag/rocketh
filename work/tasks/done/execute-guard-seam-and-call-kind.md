...
- **Option name `guard`, member `kind: 'call'`, verdict `satisfied`.** Fixed by the spec and ADR 0013; recorded only for completeness. `equals`, output selection and the `storage` kind are deliberately absent, they are the sibling tasks.
- **Result discriminant is `outcome: 'skipped' | 'sent'`, types `GuardedExecutionResult = SkippedExecution | SentExecution`.** I chose `outcome` over `status` because `status` already means the receipt's success flag one level down (`result.receipt.status`), and two `status` fields on nested objects meaning different things is exactly the re-meaning the coherence check warns about. `skipped` is not named after a transaction, per the task. Touches: `execute-guard-storage-kind`, `-equals-and-output-selection`, `-evaluation-is-legible` and `-convergence-scenario` all consume these names.
- **The evaluator lives in a new `src/guard.ts`, and `read` moved to a new `src/read.ts`.** The evaluator must go _through_ `read`, and `execute` (in `index.ts`) must call the evaluator, so leaving `read` in `index.ts` would have made `index ↔ guard` a cycle. `read.ts` is a verbatim move (no behaviour change), and the root still exports `read`/`readByName` identically. Alternative considered: keep everything in `index.ts`, rejected because three sibling tasks extend the guard and would pile into an already-long module.
- **`evaluateGuard(env)(guard, defaultTarget?)` throws when the guard names no `on` and no default target is given.** This is a NEW refusal, reachable only from standalone evaluation (`execute` always passes the executed deployment), and it seemed clearly better than guessing or silently reading address zero. Message names the guard's function. Alternative considered: make `on` mandatory in the type, rejected because "defaults to the contract being executed" is an acceptance criterion.
- **The unguarded call signature is `ExecutionArgs & {guard?: undefined}`.** This accepts everything the pre-guard signature accepted (the return type is pinned as _exactly_ unchanged), but it makes a _possibly_-present guard (`guard: maybeGuard`) match neither signature, i.e. a compile error, rather than silently typing as an unguarded call whose receipt a skipped result would not have. Alternative considered: leave the second signature untouched, which types such a call as returning a receipt and is unsound at runtime. Touches user-visible ergonomics: a caller who conditionally attaches a guard must branch instead.
- **The guard reads at `latest` only.** It takes `on`, `functionName`, `args` and `satisfied`, and deliberately not `read`'s `blockTag` / `blockNumber` / `account`. A guard asking "is this call still needed" against a historical block is close to meaningless, and admitting the fields now would make them API. Additive later if a real case appears.
- **The guard is evaluated before `env.resolveAccount(account)`.** So a satisfied guard costs one read and nothing else, and cannot fail on account resolution for work that is already done.
- **I added a README section for the guard** (and corrected "The four functions", which my change made wrong). The dedicated docs page belongs to `document-execute-state-guard`; I kept the README entry short so that task can link rather than fight it.
