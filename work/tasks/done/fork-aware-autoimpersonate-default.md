...
_Where the "nobody configured it" absence is preserved: in `ChainSemantics` only, not in the published `ChainConfig`._ The chain term always won because it defaulted itself to `false`, so the fork-aware default could not be reached. I made `ChainSemantics.autoImpersonate` optional (`packages/rocketh/src/environment/chains.ts`, an internal type of the `rocketh` package that is not exported from its root) and re-closed the absence inside `getChainConfigFromUserConfig`, so `ChainConfig.autoImpersonate` stays a plain required `boolean`. Alternatives considered: (a) make `ChainConfig.autoImpersonate` optional in `@rocketh/core`, which is the more coherent end state and mirrors the existing `onUnknownSigner` treatment, but is a breaking type change to a published core type for a widening nothing in this repo reads, and `AGENTS.md` says ask first; (b) re-read the raw `config.chains[id]` in the executor to detect the absence, which forks the "read a chain's semantics" concept into two readers. This touches `getChainConfigFromUserConfig`'s callers only in that their `autoImpersonate` keeps the value it always had. If someone later wants one meaning throughout, (a) is the follow-up, and the JSDoc at the choice site says so.

_Dropping the `simulatedChainId === connectedChainId` shortcut._ The previous task read the simulated semantics off the already-built connected `ChainConfig` when the two ids coincide. That is every non-fork run and, crucially, a fork under hardhat (31337 for both), and it would have re-injected the defaulted `false` and made the new default dead exactly where the task warned. The two paths produce identical values for every other field (`getChainConfigFromUserConfig` spreads precisely those semantics), so the only change is the one intended. Cost is one extra cheap object construction per run.

_The default keys off fork-ness alone, not off knowing the simulated chain id._ `autoImpersonate = fork !== undefined`. A fork whose chain id was never established (hardhat with nothing declared) still gets impersonation on, because the reason for the default is that the run is a rehearsal, not that we know which network it rehearses. The alternative, requiring `fork.chainId`, would make the default silently absent in the very setup most hardhat-deploy users are on. This touches no other flag or command; ADR 0014 already states the consequence in these terms, so no new ADR was written.

_Correcting one sentence in the production-hardening page rather than documenting the feature._ `documentation/production-hardening/index.md` stated impersonation is "off by default" without qualification, which this change makes conditionally false. The fork documentation itself belongs to `document-fork-runs` (still in backlog, and its acceptance criteria explicitly cover the impersonation default), so I limited myself to making the existing sentence true and left the new page to that task.
