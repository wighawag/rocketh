...
- **An unreachable node FAILS the run; it does not fall back to the declared id.** The only fallback available is `environments[<name>].chain`, and on a fork that value routinely names the _simulated_ network (the docs tell a hardhat user to declare `chain: 1` so mainnet's settings are found), so falling back would silently hand the _connected_ identity a simulated id and every transaction would be signed for a chain the node rejects. That is exactly the bug `fork-chain-identity-simulated-versus-connected` closed. A fork run that cannot reach its node can do nothing useful anyway, so stopping loses nothing. The alternative considered was falling back to the declared id with a warning, rejected for the reason above. Both error messages name the endpoint that was tried and the message for an unreachable node points at `environments.<name>.whenForked.rpcUrl`. This introduces a new refusal on a path that previously threw anyway (it threw `Could not find chainId ... (no provider)` before dialling), and it touches the future `--is-fork` flag: `rocketh -e mainnet --is-fork` against a node that is not up will fail with the endpoint named rather than proceeding on a declared id.
- **A node answer that is not a usable chain id is treated as the same failure.** `0`, `NaN` or a non-integer throws naming the endpoint, rather than falling through the `||` to the declared id, which would reopen the same wrong-identity path by a side door. Alternative considered: leave the existing `||` semantics and let a nonsense answer fall back. Rejected as the cheaper-looking but more dangerous branch.
- **The shared endpoint resolution lives in `packages/rocketh/src/executor/index.ts` as `resolveForkRunEndpoint`, next to `CONVENTIONAL_LOCAL_RPC_URL`,** which I moved up in the file so both consumers read after its definition. That module already owns both callers (`getChainIdForEnvironment` and `resolveExecutionParams`) and already owned the constant, so no new module or dependency edge was needed. The alternative considered was `packages/rocketh/src/environment/chains.ts` alongside `getChainConfigFromUserConfig`; rejected because that module is keyed by chain id and knows nothing about environments or fork runs, so the fork-only rule would have sat at the wrong layer. It is exported from the executor module (like its neighbours) but deliberately NOT re-exported from the package root `packages/rocketh/src/index.ts`: nothing outside needs it yet, and the resolved endpoint is already observable on `resolvedExecutionParams.provider`.
- **Naming: `resolveForkRunEndpoint`, not `resolveForkRpcUrl`.** `CONTEXT.md` and ADR 0014 pin that the bare word _fork_ is ambiguous across three parts of speech and that a `FORK_RPC_URL` reads as the url to fork FROM, which is the one thing this is not. _fork run_ is the existing glossary term for the adjective reading, so the name reuses it rather than coining a new concept, and the JSDoc says so at the choice site.
