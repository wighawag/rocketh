...
- **The page is `documentation/script-lifecycle/` ("Running Once, and Running Last" in the sidebar), not "Migrations".** The word _migration_ already means porting from hardhat-deploy v1 in this docs site (`documentation/migration/`, and the `unknown-signer-v1-migration` spec), so a page named after `.migrations.json` would make one term mean two things in the same sidebar. I kept **migration record** as the code-level name (it is what the file, `hasMigrationBeenDone` and `recordMigration` are called) and used **run-once script** as the user-facing name, which is the phrase already shipped in `documentation/unknown-signers/index.md` ("the run-once mechanism"). Alternatives considered: `documentation/migrations/` (rejected, re-means an existing term) and folding the content into `core-concepts` (rejected, the deferral hazard needs a page-level warning). This touches the CONTEXT.md glossary, where I added an entry pinning the two senses apart so the next task does not re-fork them.
- **Both features share one page rather than getting one each.** They meet in the motivating example: a `runAtTheEnd` batch proposer sits in exactly the script shape where an unconditional `return true` loses a step, so splitting them would duplicate the warning or leave one page silently incomplete. Cost: the page carries two headline concepts, mitigated by the two top-level sections and the option table at the top.
- **The `saveDeployments` sentence is stated conditionally, not as "in-memory runs do not save".** The resolution in `packages/rocketh/src/executor/index.ts` is: a fork defaults to not saving, a run with NO supplied provider defaults to saving whatever the environment is called, and only a run with a supplied provider treats `memory` / `hardhat` / `default` as non-saving. The simpler wording would have been wrong for `rocketh -e memory`, so the bullet names the provider condition.
- **Empty changeset**, following the `document-captured-transactions` precedent for a docs-only change: it touches no package, and an empty entry keeps `changeset status --since=main` honest without queueing a release. Per CONTEXT.md it must be consumed by a real changeset before publishing, which the existing non-empty entries already provide.
