...
1. **The field is `env.capturedTransactions`, and the entry type is `CapturedTransaction`.** Chosen for coherence with the language the spec and the two follow-on tasks already use (_captured transactions_, and `document-captured-transactions` asks for a `CONTEXT.md` glossary entry for _a captured transaction_ distinguishing it from the pending-transaction record). Alternatives considered and rejected: `transactions` (too generic beside `network.provider.transactionHashes`, and it says nothing about the run-scoped, what-we-DID meaning), `broadcastTransactions` (reads as a verb, the same ambiguity `CONTEXT.md` pins against for `--fork` vs `--is-fork`), and `broadcasts` (loses the noun the consumers actually want). It touches the file-sink task (which serialises this list) and the documentation task (which pins the term), so both inherit this spelling.

2. **`data` is OPTIONAL on the intent arm.** The shape block quoted in the task types it as required (`data: 0x${string}`), while the same task's body requires the deterministic-factory funding transfer, which genuinely carries none, to have _no `data` key at all_. `EIP1193TransactionData.data` is itself optional (`eip-1193@0.6.5`, `BaseEIP1193TransactionData`), so a required field would have been a lie the compiler could not keep. I followed the body and the acceptance criterion (absent stays absent) over the literal type block. This touches the file-sink task: a serialiser must handle an intent entry with no `data`, and must not substitute `'0x'` (which a replay would send as an empty call rather than a plain transfer).

3. **Capture happens immediately after the send RPC resolves, BEFORE the `autoMine` `evm_mine` call.** The truth being recorded is "the run sent this", and the node accepted it at that point; if a subsequent `evm_mine` fails the run throws with the entry already present, which is correct, because the transaction really was sent. The alternative (capture after `autoMine`, just before each `return`) would drop an entry for a transaction that was genuinely broadcast. This also fixes the concurrency promise concretely: the order is the order the run observed the _sends complete_.

4. **On the `ask` path the entry is pushed at one single site, after the pasted transaction has been found, checked successful and (for a deployment) checked to have deployed something, just before the hash is returned.** Every earlier exit from that branch either defers (`throw` policy, "cannot sign", a paste the user would not vouch for) or fails verification, so none of them can leak an entry, and the successful path cannot produce two. What is recorded is the intent _rocketh asked for_, not the transaction the human actually sent, since a Safe execution goes to the Safe carrying the call inside it and is not replayable as such.

5. **The array is exposed by IDENTITY, typed `readonly CapturedTransaction[]`.** A deploy script therefore holds the same object and can read the list mid-run. That is the accepted consequence the spec ratifies, not a feature: I added no hook, no callback, no second capture-aware entry point, and documented nothing about mid-run reads. The `readonly` element type is the cheap half-measure that stops a consumer pushing into the run's own list at the type level.

6. **The funnel that does NOT reach the choke point, confirmed in code: `recoverTransactionsIfAny`** (`packages/rocketh/src/environment/index.ts`, the `internal` handle, run before the scripts). It adopts transactions a _previous_ run broadcast, waits for their receipts and saves the resulting deployments, so those transactions never pass this run's choke point and are absent from this run's list. I deliberately did not widen the seam: they are not this run's intent, and a fork run does not save, so it has no pending-transaction file to recover from. This is the one case where the list is not the whole of what this run's node saw happen, and I stated it in the JSDoc on `capturedTransactions` so the next reader meets it at the type. Verified as the _only_ such gap: the only three send sites in any package's `src` (`eth_sendTransaction`, and `eth_sendRawTransaction` twice) all live inside the choke point and all now capture.

7. **The tests build the environment through `loadEnvironmentFromStore` rather than calling `createEnvironment` directly**, unlike the sibling seam tests. That is what lets one helper cover both a memory run and a _fork run_ (the fork environment name and its chain identity are derived there, per ADR 0014) without the test hand-rolling the naming, which would have been a second place to keep in step with the fork rules.
