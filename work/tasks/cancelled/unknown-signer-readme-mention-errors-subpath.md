...
> **CANCELLED, and the work is DONE — this is not an abandonment.** `packages/rocketh-unknown-signer/README.md` gained a "Getting at the error type" section showing `import {UnknownSignerError} from '@rocketh/unknown-signer/errors'`, and explaining why the root stays function-only: every root export is called as `value(env)`, so a class there is refused by name at deploy-script run time.
>
