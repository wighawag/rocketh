...
**After this task**, the run either is not a fork, or is a fork of a NAMED network whose chain id is known, and both facts are readable from the environment. `if (env.network.fork)` must still read naturally, and a non-fork must be falsy, because that is how the existing consumers are written.
...
- [ ] A run given a fork input reports WHICH network is forked, and that network's chain id, through the environment
