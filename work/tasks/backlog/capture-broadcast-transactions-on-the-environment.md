...
1. **How should an ALREADY-SIGNED broadcast (`TransactionToBroadcast` variant `{type: 'raw'}`) appear in the captured list?** The spec commits to capturing INTENT and puts "raw signed transactions" in Out of Scope, but a run really does broadcast one: `@rocketh/deploy` sends the pre-signed deterministic-deployment factory transaction (Nick's method) whenever the Create2 factory is absent, which is exactly the fresh-node case stories 5 and 6 describe. Three options, each in tension with a different story: (a) capture it as its own entry variant carrying the raw payload and `from`, which replays verbatim but embeds a nonce and a gas price inside the blob (cuts against story 10, "nothing a consumer should not replay"); (b) decode it into intent, which then replays from a different sender/nonce and therefore lands the factory at a DIFFERENT address, breaking story 6's address equality, since the canonical factory address is derived from that exact sender and nonce; (c) omit raw broadcasts, which makes the list silently incomplete and leaves a replay unable to deploy the factory at all (cuts against story 9, "no future send path can silently escape it"). Which of the three, and if (a), what does the entry look like?
2. **Where does `account` come from, and what happens when it is ambiguous?** The choke point receives a transaction, not the named account the call site used, so the only in-place source is a reverse lookup from `from` over the run's named accounts. Several named accounts commonly resolve to the SAME address (`{deployer: {default: 0}, admin: {default: 0}}`), so "the" name is genuinely ambiguous. Pick the first match deterministically (mirroring the existing deployment-name enrichment at this same seam, which takes `names[0]` and says why), emit every matching name, or thread the resolved account name down from the call sites (a wider change through every package that broadcasts, and it contradicts the spec's "there is no new seam")? Story 2 is "which Safe does this transaction belong to", so the ambiguous case is the case that matters.
3. **Is a transaction resolved INTERACTIVELY (the `ask` path, where a human executed it out-of-band and pasted the hash back) an entry in this list?** It passes through the same choke point and the run observed its receipt, so recording it is consistent with ADR 0012's "a record asserts only what rocketh observed", and a replay of the run's effects needs it. But rocketh did not send it, and a Safe-batch consumer that re-proposes something the operator has already executed would double-execute. Capture it, or skip it?
4. **In what form does an entry carry `value`?** The spec's shape says `value?: bigint`, but the choke point never sees a bigint: the object variant of `TransactionToBroadcast` carries `EIP1193TransactionData`, whose `value` is a 0x-QUANTITY string, and every call site already builds it that way (the Create2 factory funding transfer in `packages/rocketh-deploy/src/index.ts` sets `value: '0x...'`; `packages/rocketh-read-execute/src/index.ts` does `` `0x${viemArgs.value.toString(16)}` ``). A bigint field therefore means capture DECODES the wire form and the file sink RE-ENCODES it, and it makes the list non-serialisable by a plain `JSON.stringify` (a bigint throws), which an in-process consumer hits before any file exists. Keep the 0x-quantity form (uniform with `from` / `to` / `data`, serialisable as-is, and the same bytes the file would carry), or keep the spec's bigint for in-process ergonomics and encode at the file boundary? It is ONE decision for both the in-memory list and the file, so it belongs here and not in the file task.
