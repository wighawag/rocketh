...
Worth recording: this was originally the second half of `execute-state-guard` and was split out on three grounds. It has a different risk profile (a persisted file format and a config key, versus an option on a function). It maps to a different scope tier (deferred, where the guard sits inside the committed scope), so keeping them together would have made committed work wait on deferred design. And the guard turned out to be larger than expected after its paper validation, which found a required second guard kind. The connection is preserved by the fact that both answer one sentence from one user, quoted in both specs.
