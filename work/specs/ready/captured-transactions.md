...
**The Solidity consumer has a second route, but it costs a PORT, and the port-free version of it is blocked upstream.** This was checked rather than assumed, because the obvious reading is that capture is redundant here, and it is not.

`hardhat test solidity` does support forking: `SolidityTestForkingConfig` takes `{url, blockNumber, rpcEndpoints}`, reaching EDR's `ethRpcUrl`/`forkBlockNumber`. So a plugin CAN run the deploy scripts against a local node and point the Solidity tests at it, and the tests then see the real deployed state with no capture and no replay. But `url` is an HTTP endpoint, so this requires a live node listening on a port for the duration of the test run, which is exactly what an in-memory Solidity test run otherwise avoids.

The port-free shape would be to seed the test EVM with the deployed state directly. EDR has the field for it, `SolidityTestRunnerConfigArgs.localPredeploys: Array<AccountOverride>`, and it is the ONLY state-seeding field on that config (`runSolidityTests` takes config args, not a provider, so there is no way to hand it an existing instance either). `AccountOverride` carries `address`, `balance`, `nonce` and `code`, and its `storage` field is documented in EDR 0.15.0 as **"not supported yet"** (NomicFoundation/edr#911).

Without storage that route is not merely limited, it is useless for this purpose: a deployed contract injected as code with empty storage has no owner, no initialised state and, for a proxy, no implementation slot. So today the choice for a Solidity fixture is a live node on a port, or replaying captured transactions in `setUp()`, and only the second is port-free. Replay also works under pure `forge` with no hardhat and no RPC at all, and produces a checked-in artifact.

Watch NomicFoundation/edr#911: if `storage` lands, seeding the test EVM from a deploy run becomes strictly better than both routes, and the Solidity justification for capture weakens accordingly.

Even so, **the load-bearing justification is the Safe batch**, which has no alternative route at all. If this spec is ever cut down, cut it to that.
