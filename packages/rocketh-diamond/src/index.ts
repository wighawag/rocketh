...
export async function diamond<TAbi extends Abi>(
	env: Environment,
	name: string,
	params: DiamondDeploymentConstruction<TAbi>,
	options: DiamondDeployOptions
	...
	if (proxy) {
		oldFacets = await read(env, proxy as unknown as Deployment<DiamondLoupeABI>, {
			functionName: 'facets',
		});
	}
	...
	for (const facet of facetsSet) {
		...
		const implementation = await deploy<typeof artifact.abi>(
			env,
			facetName,
			{
				...params,
	...
	if (options.execute) {
		...
		if (options.execute.type === 'artifact') {
			const executionDeployment = await deploy(
				env,
	...
	if (changesDetected) {
		if (!proxy) {
			...
			if (initializationsArgIndex >= 0 || erc165InitArgIndex >= 0) {
				// TODO:TMP
				const diamondERC165InitDeployment = await deploy(
					env,
			...
			const currentOwner = await read(env, proxy as unknown as Deployment<OwnershipFacetABI>, {
				functionName: 'owner',
			});
			...
			const txHash = await execute(env, proxy as unknown as Deployment<DiamondCutABI>, {
				...params,
