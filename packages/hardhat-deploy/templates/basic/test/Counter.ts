...
describe('Counter', async function () {
	it('Should emit the Increment event when calling the inc() function', async function () {
		...
		const CounterContract = env.viem.getWritableContract<Abi_Counter>('Counter', {
			account: env.namedAccounts['deployer'],
		});
		await viem.assertions.emitWithArgs(CounterContract.write.inc(), CounterContract, 'Increment', [1n]);
	});
