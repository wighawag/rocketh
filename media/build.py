...
def pilot_net_tilt(svg_text):
    """Rotation the pilot actually ends up with, read back from the output.

    Composes the outer mark matrix with the pilot group's own transform. This is
    measured from the emitted file rather than inferred from the inputs, so it
    still fails if the counter-rotation is dropped or mis-signed.
    """
    outer = re.search(r'transform="matrix\(([^)]*)\)"', svg_text).group(1).replace(",", " ").split()
    a, b = float(outer[0]), float(outer[1])
    inner = re.search(r'scale\([\d.]+\)\s*rotate\(([-\d.]+)', svg_text)
    inner_deg = float(inner.group(1)) if inner else 0.0
    return math.degrees(math.atan2(b, a)) + inner_deg
...
def build_hd_logo():
    ...
    # Cancel the mark's pose so the pilot stays UPRIGHT. She sits inside the
    # rotated group, so without this she leans with the hull. The original mark
    # did the same thing: her wrapper carried a -40.24 deg rotation against the
    # outer +40.24 deg, netting zero. The angle is read back out of the matrix
    # rather than hardcoded, so re-posing the mark keeps her level.
    angle = math.degrees(math.atan2(float(matrix.split("(")[1].split()[1]),
                                    float(matrix.split("(")[1].split()[0])))
    counter = f" rotate({-angle:.4f} {pw/2:.3f} {ph/2:.3f})"
    out = (
        ...
        f'    <g transform="translate({tx:.3f},{ty:.3f}) scale({scale:.6f}){counter}">\n'
        f"{pbody}\n    </g>\n  </g>\n</svg>\n"
    )
    try:
        ET.fromstring(out)
    ...
    net = pilot_net_tilt(out)
    if abs(net) > 0.01:
        sys.exit(f"pilot is tilted {net:+.2f} deg; she must sit upright in the porthole")
    print(f"wrote {OUT_HD_LOGO.relative_to(ROOT)}  ({OUT_HD_LOGO.stat().st_size} bytes)  "
          f"refs ok: {sorted(refs)}  mark pose {angle:+.1f} deg, "
          f"pilot net tilt {net:+.2f} deg")
